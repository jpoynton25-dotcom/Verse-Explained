...
const hub = getHubData("/verse-meanings");
...
export default function VerseMeaningsHubPage() {
  return (
    <>
      <SchemaScript id="verse-meanings-breadcrumb-schema" data={breadcrumbSchema(hub.breadcrumb.map((item) => ({ name: item.label, path: item.href })))} />
      <SchemaScript
        id="verse-meanings-collection-schema"
      ...
      {hub.childGroups.map((group) => {
        ...
        return (
          <SchemaScript
            key={group.heading}
            id={"verse-meanings-itemlist-" + group.heading.toLowerCase().replaceAll(" ", "-")}
            data={itemListSchema({ name: group.heading, path: hub.path, items })}
          />
        );
      })}
      {hub.faqs.length ? <SchemaScript id="verse-meanings-faq-schema" data={faqSchema(hub.faqs)} /> : null}
      <HubPage hub={hub} />
    </>
  );
}
