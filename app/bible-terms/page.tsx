...
const hub = getHubData("/bible-terms");
...
export default function BibleTermsHubPage() {
  return (
    <>
      <SchemaScript id="bible-terms-breadcrumb-schema" data={breadcrumbSchema(hub.breadcrumb.map((item) => ({ name: item.label, path: item.href })))} />
      <SchemaScript
        id="bible-terms-collection-schema"
      ...
      {hub.childGroups.map((group) => {
        ...
        return (
          <SchemaScript
            key={group.heading}
            id={"bible-terms-itemlist-" + group.heading.toLowerCase().replaceAll(" ", "-")}
            data={itemListSchema({ name: group.heading, path: hub.path, items })}
          />
        );
      })}
      {hub.faqs.length ? <SchemaScript id="bible-terms-faq-schema" data={faqSchema(hub.faqs)} /> : null}
      <HubPage hub={hub} />
    </>
  );
}
